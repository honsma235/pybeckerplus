...
PATTERNS = {
    "status": re.compile(
        r"^0700011A(?P<mac>.{16}).{14}80.{2}(?P<rssi>.{2})(?P<status>.{4})(?P<pos>.{4})(?P<cnt>.{4}).{4}$",
        re.IGNORECASE,
    ),
    "unsolicited": re.compile(
        r"^07000126(?P<mac>.{16}).{14}52.{2}(?P<rssi>.{2})(?P<status>.{4})(?P<pos>.{4}).{32}$",
        re.IGNORECASE,
    ),
    "info": re.compile(
        r"^0700012B(?P<mac>.{16}).{14}51.{18}(?P<sn>.{10}).{2}(?P<fw>.{6}).{10}(?P<cnt>.{4}).{4}$",
        re.IGNORECASE,
    ),
    "name": re.compile(
        r"^07000130(?P<mac>.{16}).{14}62(?P<name>.{64})$", re.IGNORECASE
    ),
    "parent_mac": re.compile(
        r"^0700011A(?P<mac>.{16}).{6}(?P<root>.{8})83(?P<parent>.{8}).{4}(?P<cnt>.{4}).{4}$",
        re.IGNORECASE,
    ),
    "stick_info": re.compile(
        r"^07270111(?P<mac>.{16})(?P<install>.{8}).{10}$", re.IGNORECASE
    ),
    ...
    for ptype, pattern in PATTERNS.items():
        ...
        if ptype == "parent_mac":
            return {
                "type": "device",
                "mac_id": match.group("mac").lower(),
                "parent_mac": match.group("root").lower()
                + match.group("parent").lower(),
        ...
        if ptype == "stick_fw":
            fw_bytes = hex_to_bytes(match.group("fw"))
            return {"type": "stick_fw", "fw": ".".join([f"{b:02}" for b in fw_bytes])}
